...
using System.Text;
using SGL.Nodes.StaticMethod;
using SGL.Types;

namespace SGL.Nodes
{
    class ObjectMethodCallNode : INode
    {
        ...
        Dictionary<String, ObjectMethod> objectMethods;
        ...
        public ObjectMethodCallNode(String objectVar, List<INode> indexes, String identifier, List<INode> parameters, Dictionary<String, Method> methods, Dictionary<String, ObjectMethod> objectMethods, StringBuilder sb, Scope globalScope, Scope currentScope, Random random, int line)
        {
        ...
        public Value Evaluate()
        {
            ...
            // Get the SGLValue
            // simple lookup
            Value sbObject = currentScope.Resolve(objectVar);

            if (indexes.Count >= 1)
            {
                // a possible further list-lookup
                ...
                {
                    if (!index.IsNumber() || !(sbObject.IsList()))
                    {
                        //throw new CompilerException(GetLine(), "illegal expression", "illegal expresson '" + objectVar + "[" + index + "]" + "'");
                    }
                    ...
                    if (sbObject.IsList())
                    {
                        sbObject = sbObject.AsList()[idx];
                    }
                }
            }
            // now sbObject should point at the requested object for the method

            // but is it an object?
            if (!sbObject.IsObject())
            {
                //throw new CompilerException(GetLine(), "type missmatch", "the object method '" + identifier + "' can't be used on the non-object " + objectVar);
            }
            ...
            try
            {
                Value val = IsStaticMethod(sbObject, currentScope.GetOffset(), identifier, paramValues, currentScope, sb);
            ...
            if (!objectMethods.ContainsKey(identifier))
            {
                // TODO: arguments?
                //throw new CompilerException(GetLine(), "method undefined", "the method '" + identifier + "' is undefined or is not applicable for the arguments ()");
                
            }

            // ObjectMethod is not static (user-defined) and exists
            ObjectMethod om = objectMethods[identifier];

            //ObjectMethod objectmethod = new ObjectMethod(om);

            return Value.NULL;
            //return objectmethod.Invoke(sbObject, currentScope.GetOffset(), parameters, methods, objectMethods, sb, globalScope, random);
        }

        private Value IsStaticMethod(Value sprite, int offset, String method, List<Value> values, Scope scope, StringBuilder sb) {
            /*
            ...
            */
            ...
            else if (method.Equals("fade"))
            {
                ...
                else if (values.Count == 2)
                {
                    ...
                    AddCommandToList(obj, new SbAnimation(SbAnimation.AnimationType.fade, 0, time, time, startParams, startParams));
                ...
                else if (values.Count == 5)
                {
                    ...
                    AddCommandToList(obj, new SbAnimation(SbAnimation.AnimationType.fade, easing, stime, etime, startParams, endParams));
            ...
            else if (method.Equals("scale"))
            {
                ...
                else if (values.Count == 2)
                {
                    ...
                    int time = offset + values[0].AsInteger();
                    double[] startParams = new double[] { values[1].AsFloat() };
                    AddCommandToList(obj, new SbAnimation(SbAnimation.AnimationType.scale, 0, time, time, startParams, startParams));
                ...
                else if (values.Count == 5)
                {
                    ...
                    double[] startParams = new double[] { values[3].AsFloat() };
                    double[] endParams = new double[] { values[4].AsFloat() };
                    AddCommandToList(obj, new SbAnimation(SbAnimation.AnimationType.scale, easing, stime, etime, startParams, endParams));
            ...
            else if (method.Equals("rotate"))
            {
                ...
                else if (values.Count == 2)
                {
                    ...
                    int time = offset + values[0].AsInteger();
                    double[] startParams = new double[] { values[1].AsFloat() };
                    AddCommandToList(obj, new SbAnimation(SbAnimation.AnimationType.rotate, 0, time, time, startParams, startParams));
                ...
                else if (values.Count == 5)
                {
                    ...
                    double[] startParams = new double[] { values[3].AsFloat() };
                    double[] endParams = new double[] { values[4].AsFloat() };
                    AddCommandToList(obj, new SbAnimation(SbAnimation.AnimationType.rotate, easing, stime, etime, startParams, endParams));
            ...
            else if (method.Equals("moveX"))
            {
                ...
                else if (values.Count == 2)
                {
                    ...
                    AddCommandToList(obj, new SbAnimation(SbAnimation.AnimationType.moveX, 0, time, time, startParams, startParams));
                    obj.x = values[1].AsInteger();
                ...
                else if (values.Count == 5)
                {
                    ...
                    AddCommandToList(obj, new SbAnimation(SbAnimation.AnimationType.moveX, easing, stime, etime, startParams, endParams));
                    obj.x = values[4].AsInteger();
            ...
            else if (method.Equals("moveY"))
            {
                ...
                else if (values.Count == 2)
                {
                    ...
                    int time = offset + values[0].AsInteger();
                    double[] startParams = new double[] { values[1].AsInteger() };
                    AddCommandToList(obj, new SbAnimation(SbAnimation.AnimationType.moveY, 0, time, time, startParams, startParams));
                    obj.y = values[1].AsInteger();
                ...
                else if (values.Count == 5)
                {
                    ...
                    double[] startParams = new double[] { values[3].AsInteger() };
                    double[] endParams = new double[] { values[4].AsInteger() };
                    AddCommandToList(obj, new SbAnimation(SbAnimation.AnimationType.moveY, easing, stime, etime, startParams, endParams));
        ...
        private void AddCommandToList(SpriteObject obj, SbCommand cmd)
        {
            if (obj.loop)
            {
                ...
                {
                    // remove offset
                    SbAnimation anim = (SbAnimation)cmd;
                    anim.startTime -= this.offset;
                    anim.endTime -= this.offset;
